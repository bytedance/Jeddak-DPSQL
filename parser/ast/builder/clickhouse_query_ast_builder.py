...
from parser.ast.dialect.clickhouse.clickhouse_lexer import ClickHouseLexer
from parser.ast.dialect.clickhouse.clickhouse_parser import ClickHouseParser
from parser.ast.dialect.clickhouse.clickhouse_visitor import ClickHouseVisitor
from parser.ast.statement.clickhouse_block_statement import FormatBlockStatement
from parser.ast.expr.clickhouse_expr import TimeStampExpression, \
    TrimFunction, Interval, ExtractFunction, FormatExpr
from parser.ast.dialect.clickhouse.error_listener import SyntaxErrorListener
from parser.ast.expr.agg_function_expr import SqlSumFunction, SqlAggFunction, SqlCountFunction, SqlMinFunction, \
    SqlMaxFunction, SqlAvgFunction, SqlVarPopFunction, SqlStdPopFunction
from parser.ast.expr.arithmetic_expr import MultiplyExpression, DivideExpression, ModuloExpression, \
    AdditionExpression, SubtractionExpression, NegateExpression
from parser.ast.expr.base_expr import NameExpression, Identifier, Column, Number, NestedExpression, Literal, \
    AllColumns, Table, ArrayAccess, Arrays, Tuples, TupleAccess, LambdaExpression, TableFunctionExpr, SelectItem, \
    MapExpression
from parser.ast.expr.cond_expr import MultiIfExpression, CaseExpression, WhenExpression, TernaryExpression, \
    IfExpression, IIFFunction
from parser.ast.expr.function_expr import CommonFunction, PowerFunction
from parser.ast.expr.logical_bool import LogicalEqual, BinaryEqual, LogicalCompare, LogicalIn, LogicalLike, \
    LogicalConcat, LogicalAnd, LogicalOr, LogicalNot, LogicalIsNull, LogicalBetween
from parser.ast.expr.other import LimitExpr, JoinConstraintExpr, OrderExpr, EnumExpr, DateExpr, CastExpr, \
    TokenDefinitionExpr, SampleExpr, ValueTypeDefinitionExpr
from parser.ast.statement.block_statement import SelectBlockStatement, FromBlockStatement, WhereBlockStatement, \
    GroupByBlockStatement, LimitBlockStatement, OrderByBlockStatement, LimitByBlockStatement, TeaLimitBlockStatement, \
    SettingsBlockStatement, HavingBlockStatement, ArrayJoinBlockStatement, WithBlockStatement
from parser.ast.statement.select_statement import ByteClickHouseSelectStatement, SelectUnionStatement
from parser.ast.tablesource.table_source import SqlTableSource, SqlSubquerySource, SqlTableJoinSource, \
    FusionMergeSource
from parser.ast.type.node_type import SQLSetQuantifier, ExprAliasType, NodeType, OperatorType, LogicalInType, \
    LogicalLikeType, AggSumType, LiteralType, GroupByType, LimitSplitType, JoinLocality, JoinType, JoinStrictness, \
    JoinKind, JoinConstraintType, OrderingType, TimeIntervalType, TrimType, UnionType, ArrayJoinType
from parser.ast.type.sql_type import SqlNumberType, SqlDecimalType, SqlNestedType, SqlEnumType, SqlComplexType, \
    CommonWithParamType


class ClickHouseAstBuilder(AstBuilder):
    """
        Get the ClickHouse parser, parse sql to ast.

    """
    def get_parser(self, stream):
        lexer = ClickHouseLexer(stream)
...
class QueryVisitor(ClickHouseVisitor):
    ...
    def visitSelectStmt(self, ctx: ClickHouseParser.SelectStmtContext):
        ...
        array_join_block = ArrayJoinBlockVisitor().visit(array_join_clause) if array_join_clause is not None else None
        # prewhere_clause = ctx.prewhereClause()
        ...
        limit_by_clause = ctx.limitByClause()
        limit_by_block = LimitVisitor().visit(limit_by_clause) if limit_by_clause is not None else None

        limit_clause = ctx.limitClause()
        limit_block = LimitVisitor().visit(limit_clause) if limit_clause is not None else None

        tealimit_clause = ctx.tealimitClause()
        tealimit_block = LimitVisitor().visit(tealimit_clause) if tealimit_clause is not None else None

        settings_clause = ctx.settingsClause()
        setting_block = SettingsVisitor().visit(settings_clause) if settings_clause is not None else None

        format_clause = ctx.formatClause()
        format_block = FormatVisitor().visit(format_clause) if format_clause is not None else None

        ast = ByteClickHouseSelectStatement()
        ast.with_block = with_block
        ast.select_block = select_block
        ast.source_block = from_block
        ast.array_join_block = array_join_block
        ast.where_block = where_block
        ast.groupBy_block = group_by_block
        ast.having_block = having_block
        ast.orderBy_block = order_by_block
        if limit_by_block is not None:
            ast.limit_block = limit_by_block
        elif limit_block is not None:
            ast.limit_block = limit_block
        elif tealimit_block is not None:
            ast.limit_block = tealimit_block
        else:
            ast.limit_block = None

        ast.setting_block = setting_block
        ast.format_block = format_block

        return ast


class WithBlockVisitor(ClickHouseVisitor):
...
class SelectBlockVisitor(ClickHouseVisitor):
...
class FromBlockVisitor(ClickHouseVisitor):
...
class ArrayJoinBlockVisitor(ClickHouseVisitor):
    def visitArrayJoinClause(self, ctx: ClickHouseParser.ArrayJoinClauseContext):
        array_join_block = ArrayJoinBlockStatement()
        if ctx.LEFT() is not None:
            array_join_block.array_join_type = ArrayJoinType.LEFT_ARRAY_JOIN
        elif ctx.INNER() is not None:
            array_join_block.array_join_type = ArrayJoinType.INNER_ARRAY_JOIN
        else:
            array_join_block.array_join_type = ArrayJoinType.ARRAY_JOIN

        col_list = ExprVisitor().visit(ctx.columnExprList())
        for col in col_list:
            array_join_block.array_join_list.append(col)

        return array_join_block


class WhereBlockVisitor(ClickHouseVisitor):
...
class GroupByBlockVisitor(ClickHouseVisitor):
...
class HavingVisitor(ClickHouseVisitor):
...
class LimitVisitor(ClickHouseVisitor):
    ...
    def visitLimitByClause(self, ctx: ClickHouseParser.LimitByClauseContext):
        lbbs = LimitByBlockStatement()
        limit_expr = self.visit(ctx.limitExpr())
        lbbs.limit_expr = limit_expr
        by_list = ExprVisitor().visit(ctx.columnExprList())
        for expr in by_list:
            lbbs.by_list.append(expr)

        return lbbs

    def visitTealimitClause(self, ctx: ClickHouseParser.TealimitClauseContext):
        tlbs = TeaLimitBlockStatement()
        tlbs.number = ExprVisitor().visit(ctx.numberLiteral())

        group_list = ExprVisitor().visit(ctx.groupExp)
        for expr in group_list:
            tlbs.group_list.append(expr)

        order_list = ExprVisitor().visit(ctx.orderExp)
        for expr in order_list:
            tlbs.order_list.append(expr)

        if ctx.ASCENDING() is not None:
            tlbs.ordering_type = OrderingType.ASC
        elif ctx.DESC() is not None:
            tlbs.ordering_type = OrderingType.DESC
        else:
            tlbs.ordering_type = None
...
class OrderByVisitor(ClickHouseVisitor):
    ...
    def visitOrderExpr(self, ctx: ClickHouseParser.OrderExprContext):
        ...
        # 暂未发现 DESCENDING 与 DESC具体不同点，暂按照相同处理方式
...
class SettingsVisitor(ClickHouseVisitor):
    def visitSettingsClause(self, ctx: ClickHouseParser.SettingsClauseContext):
        sbs = SettingsBlockStatement()
        settings_list = self.visit(ctx.settingExprList())
        for setting in settings_list:
            sbs.setting_list.append(setting)

        return sbs

    def visitSettingExprList(self, ctx: ClickHouseParser.SettingExprListContext):
        return [self.visit(se) for se in ctx.settingExpr()]

    def visitSettingExpr(self, ctx: ClickHouseParser.SettingExprContext):
        left = ExprVisitor().visit(ctx.identifier())
        right = ExprVisitor().visit(ctx.literal())
        be = BinaryEqual(left, right)

        return be


class FormatVisitor(ClickHouseVisitor):
    def visitFormatClause(self, ctx: ClickHouseParser.FormatClauseContext):
        fbs = FormatBlockStatement()
        fe = FormatExpr()
        fe.format_value = ExprVisitor().visit(ctx.identifier())
        fbs.format_expr = fe

        return fbs


class ExprVisitor(ClickHouseVisitor):
    ...
    def visitColumnExprAggFunction(self, ctx: ClickHouseParser.ColumnExprAggFunctionContext):
        ...
        elif func_name == "VARPOP":
            agg_function = SqlVarPopFunction()
        elif func_name == "STDDEVPOP":
...
class JoinVisitor(ClickHouseVisitor):
...
class TableExprVisitor(ClickHouseVisitor):
    ...
    def visitFushionMerge(self, ctx: ClickHouseParser.FushionMergeContext):
        database_ident = self.visit(ctx.db)
        table1 = self.visit(ctx.table1)
        table2 = self.visit(ctx.table2)
        split_column = Column(None, ExprVisitor().visit(ctx.splitcolumn))
        split_info = ExprVisitor().visit(ctx.splitinfo)
        frange = ExprVisitor().visit(ctx.frange)
        srange = ExprVisitor().visit(ctx.srange)
        ...
        fms = FusionMergeSource(database_ident, table1, table2, split_column, split_info, frange, srange, alias)
